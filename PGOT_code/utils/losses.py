...
class BraTS_DiceLoss(nn.Module):
    def __init__(self, n_classes,weights=None):
        super(BraTS_DiceLoss, self).__init__()
        self.n_classes = n_classes
        self.weight = weights
    ...
    def forward(self, inputs, target, invalid_classes=None, mask=None, softmax=False):
        """
        inputs : [B, C, D, H, W] (logits or prob)
        target : [B, D, H, W]
        invalid_classes : iterable or Tensor, e.g. [1, 3]
        ...
        '''if invalid_classes is None:
            ...
            invalid_classes = set(int(c) for c in invalid_classes)'''
        loss = 0.0
        valid_class_count = 0
        class_wise_dice = []
        if mask is not None:
