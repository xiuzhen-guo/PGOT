...
class UNetProto(nn.Module):
    def __init__(
        ...
        with open(os.path.join('./code/configs/', config), 'r') as f:
    ...
    def warm_up(self, x):
        ...
        out_seg, representation = self.backbone3d(x)
    ...
    def forward(
        ...
        return_dict = {}
        out_seg_3d, feature3d = self.backbone3d(x)
        return_dict["cls_seg_3d"] = out_seg_3d
        return_dict["feature3d"] = feature3d

        b,c,h,w,d = feature3d.shape
        ...
        if mode == "test":
            ...
            if torch.equal(tmp, self.prototypes_mu) and torch.equal(tmp,self.prototypes_sigma):
                ...
                flag = self.initialize(feature3d, label, self.nclasses) 
    ...
    def initialize(self, ft_3d, label, num_classes, low_ratio=0.2, sigma=0.2):
        ...
        for i in range(num_classes):
            if (label==i).sum()>0:
                ...
                zero_proto = torch.zeros(32, device=l_ft.device)  
                protos_mu_curr.append(zero_proto.unsqueeze(0))  
                protos_sigma_curr.append(zero_proto.unsqueeze(0))       
    ...
    def prototype_update(self, ft_3d, label, num_classes, mask=None, confidence=None):
        ...
        return self.prototype_learning_brats2024(ft_3d, label, mask, confidence, num_classes)
