...
class UNetProto(nn.Module):
    ...
    def warm_up(self,
        ...
        out_seg_2d, feature2d, _  = self.backbone(x)
        return out_seg_2d
    ...
    def forward(
        ...
        return_dict = {}
        out_seg_2d, feature2d, _ = self.backbone(x)
        return_dict["cls_seg"] = out_seg_2d

        b, dim, h, w = feature2d.shape
        out_feat = rearrange(feature2d, "b c h w -> (b h w) c")
        ...
        if torch.equal(tmp, self.prototypes):
            ...
            if not flag:
                return_dict["proto_seg"] = out_seg_2d
                return return_dict
    ...
    def prototype_update(self,
        ...
        label_expand = label.view(-1)
        if mask != None:
            mask_expand = mask.view(-1)
        else:
            mask_expand = torch.ones_like(label_expand).cuda()
