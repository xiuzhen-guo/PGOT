...
def net_factory(net_type="unet", in_chns=1, class_num=2, mode = "train", tsne=0):
    ...
    if net_type == "vnet" and mode == "train" and tsne==0:
        net = VNet_prob(n_channels=in_chns, n_classes=class_num, normalization='batchnorm', has_dropout=True).cuda()
    if net_type == "vnet" and mode == "test" and tsne==0:
        net = VNet_prob(n_channels=in_chns, n_classes=class_num, normalization='batchnorm', has_dropout=False).cuda()
    return net
