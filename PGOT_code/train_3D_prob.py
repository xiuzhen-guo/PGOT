...
def compute_probability(config, proto_mu, proto_sigma, feature3d, optimization_num=1,):
    ...
    d_h = torch.load(
        f"{ot_dir}/h_best_{optimization_num}.pt",
        weights_only=True
    ).to(device)   # (b,)
    # ---------------- load anchor ----------------
    anchor_data = torch.load(
        os.path.join(self_snapshot_path + "features/", "anchor_features.pt"),
        weights_only=True)
    anchor_ft = anchor_data["anchor_features"].to(device)  # (b, d)
    
    # ---------------- build prototypes ----------------
    Ca = config["bat_size_sr"]
    d = proto_mu.shape[1]

    proto_list = []
    for c in range(num_class):
        proto_c = (
            torch.randn(Ca // num_class, d, device=device)
            * proto_sigma[c]
            + proto_mu[c]
        )
        proto_list.append(proto_c)
    prototypes = torch.cat(proto_list, dim=0)  # (Ca, d)

    # prototype ¡ú class id
    sr_class_id = torch.arange(
        num_class, device=device
    ).repeat_interleave(Ca // num_class)  # (Ca,)

    # ============================================================
    # prototype ¡ú class prototype
    # ============================================================
    class_proto = torch.zeros(num_class, d, device=device)
    class_proto.scatter_add_(
        0,
        sr_class_id[:, None].expand(-1, d),
        prototypes
    )
    class_proto /= (Ca // num_class)   # mean
    ...
    # ---------------- normalize ONCE ----------------
    anchor_norm = F.normalize(anchor_ft, dim=1)      # (b, d)
    class_proto_norm = F.normalize(class_proto, dim=1)  # (C, d)

    # ---------------- anchor ¡ú prototype prob ----------------
    phi_anchor = d_h + 0.5 * torch.sum(anchor_ft ** 2, dim=1)  # (b,)
    logits_p_q = prototypes @ anchor_ft.t() - phi_anchor.unsqueeze(0)
    P_p_q = F.softmax(logits_p_q / tau, dim=1)  # (Ca, b)

    # prototype ¡ú class (anchor branch)
    s_q_anchor = torch.zeros(num_class, anchor_ft.shape[0], device=device)
    s_q_anchor.scatter_add_(
        0,
        sr_class_id[:, None].expand(-1, anchor_ft.shape[0]),
        P_p_q)  # (C, b)

    # ---------------- outputs ----------------
    probabilities = torch.zeros(num_class, N, device=device)
    pseudo_labels = torch.zeros(N, dtype=torch.long, device=device)
    ...
    for start in range(0, N, chunk_size):
        ...
        # anchor similarity (cosine)
        f_norm = F.normalize(f_chunk, dim=1)
        sim = f_norm @ anchor_norm.t()  # (n, b)
        omega, max_sim_idx = sim.max(dim=1)  # (n,)

        # anchor branch (class-level, no Ca)
        s_q = s_q_anchor[:, max_sim_idx]  # (C, n)

        # feature branch (class-level, C ¡Á n)
        phi_f = (d_h[max_sim_idx] + 0.5 * torch.sum(f_chunk ** 2, dim=1))  # (n,)
        logits_f = class_proto @ f_chunk.t()      # (C, n)
        logits_f = logits_f - phi_f.unsqueeze(0)  # (C, n)
        s_f = F.softmax(logits_f / tau, dim=0)

        # fusion
        omega_ = omega.unsqueeze(0)
        tilde_s = (1.0- omega_) * s_q + omega_ * s_f
        tilde_s = tilde_s / (tilde_s.sum(dim=0, keepdim=True) + 1e-8)

        prob, label = tilde_s.max(dim=0)

        probabilities[:, start:end] = tilde_s
