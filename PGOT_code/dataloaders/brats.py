...
class BraTS2024(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        train_path = self._base_dir+'/train_1271.list'
        val_path = self._base_dir+'/val_100.list'
    ...
    def __getitem__(self, idx):
        ...
        h5f = h5py.File(self._base_dir + "/data_h5/{}.h5".format(image_name), 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        new_label = label.copy()
        new_label[(label == 1) | (label == 3)] = 1
        new_label[label == 4] = 3
        sample = {'image': image, 'label': new_label.astype(np.uint8)}
        if self.transform:
    ...
    def __call__(self, sample):
        ...
        if len(image.shape)==4:
            ...
            for i in range(image.shape[0]):  
                image_rot = np.rot90(image[i], k)
                #print("1:",image.shape)
                image_flip = np.flip(image_rot, axis=axis) 
                image_flip = np.expand_dims(image_flip,axis=0)
                image_new.append(image_flip)
            image = np.concatenate(image_new, axis=0)
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        image = sample['image']
        image = image.reshape(
            4, image.shape[1], image.shape[2], image.shape[3]).astype(np.float32)
